...
export default function RelatedQueryModelItem({node,modelIndex,labelWidth}){
    ...
    const setShowRelatedFields=()=>{
        const models=[...node.data?.models];
        models[modelIndex]={...models[modelIndex],__showRelatedFields:!(models[modelIndex].__showRelatedFields)}
        dispatch(updateNodeData({...node.data,models:models}));
    }
    ...
    const onAddRelatedField=()=>{
        const models=[...node.data?.models];
        const relatedFields=models[modelIndex].relatedFields?[...(models[modelIndex].relatedFields)]:[];
        relatedFields.push({field:"",relatedField:"",__showDetail:true});
        models[modelIndex]={...models[modelIndex],relatedFields:relatedFields};
        dispatch(updateNodeData({...node.data,models:models}));
    }
    ...
    const onDelRelatedField=(index)=>{
        const models=[...node.data?.models];
        const relatedFields=models[modelIndex].relatedFields?[...(models[modelIndex].relatedFields)]:[];
        delete relatedFields[index];
        models[modelIndex]={...models[modelIndex],relatedFields:relatedFields.filter(item=>item)};
        dispatch(updateNodeData({...node.data,models:models}));
    }
    ...
    const onRelatedModelChange=(e)=>{
        const models=[...node.data?.models];
        models[modelIndex]={...models[modelIndex],relatedModel:e.target.value}
        dispatch(updateNodeData({...node.data,models:models}));
    }
    ...
    const onRelatedFieldChange=(index,item)=>{
        const models=[...node.data?.models];
        const relatedFields=[...(models[modelIndex].relatedFields)];
        relatedFields[index]=item;
        models[modelIndex]={...models[modelIndex],relatedFields:relatedFields};
        dispatch(updateNodeData({...node.data,models:models}));
    }
    ...
    const showRelatedFields=modelItem.__showRelatedFields;
    ...
    const relatedFields=modelItem.relatedFields?.map((item,index)=>{
        return (
        <>
            <Row className="param-panel-row" style={{display:showModels&&showModel&&showRelatedFields?"flex":"none"}} gutter={24}>
                <Col className="param-panel-row-label level-3" style={{width:labelWidth}}>
                    <div className='button' onClick={(e)=>onRelatedFieldChange(index,{...item,__showDetail:!item.__showDetail})}>
                        {item.__showDetail?<MinusSquareOutlined />:<PlusSquareOutlined />}
                    </div>
                    <span>Related Field {index}</span>
                </Col>
                <Col className="param-panel-row-input" style={{width:'calc(100% - '+labelWidth+'px)'}}>
                    <Button className="button"  onClick={()=>onDelRelatedField(index)} size='small' icon={<MinusOutlined />} />
                </Col>
            </Row>
            <Row className="param-panel-row" style={{display:showModels&&showModel&&showRelatedFields&&item.__showDetail?"flex":"none"}} gutter={24}>
                <Col className="param-panel-row-label level-4" style={{width:labelWidth}}>Field</Col>
                <Col className="param-panel-row-input" style={{width:'calc(100% - '+labelWidth+'px)'}}>
                    <Input value={item.field} onChange={(e)=>onRelatedFieldChange(index,{...item,field:e.target.value})}/>
                </Col>
            </Row>
            <Row className="param-panel-row" style={{display:showModels&&showModel&&showRelatedFields&&item.__showDetail?"flex":"none"}} gutter={24}>
                <Col className="param-panel-row-label level-4" style={{width:labelWidth}}>Related Field</Col>
                <Col className="param-panel-row-input" style={{width:'calc(100% - '+labelWidth+'px)'}}>
                    <Input value={item.relatedField} onChange={(e)=>onRelatedFieldChange(index,{...item,relatedField:e.target.value})}/>
                </Col>
            </Row>
        </>
        );
    ...
    <>
        ...
        {sorter}
        <Row className="param-panel-row" style={{display:showModels&&showModel?"flex":"none"}} gutter={24}>
            <Col className="param-panel-row-label level-2" style={{width:labelWidth}}>Related Model</Col>
            <Col className="param-panel-row-input" style={{width:'calc(100% - '+labelWidth+'px)'}}>
                <Input value={modelItem.relatedModel} onChange={onRelatedModelChange}/>
            </Col>
        </Row>
        <Row className="param-panel-row" style={{display:showModels&&showModel?"flex":"none"}} gutter={24}>
            <Col className="param-panel-row-label level-2" style={{width:labelWidth}}>
                <div className='button' onClick={setShowRelatedFields}>
                    {showRelatedFields?<MinusSquareOutlined />:<PlusSquareOutlined />}
                </div>
                <span>Related Fields</span>
            </Col>
            <Col className="param-panel-row-input" style={{width:'calc(100% - '+labelWidth+'px)'}}>
                <Button className="button"  onClick={onAddRelatedField} size='small' icon={<PlusOutlined />} />
            </Col>
        </Row>
        {relatedFields}
    </>);
