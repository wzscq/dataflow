...
export default function EBProcessingNodeParams({node}){
    ...
    const bpSteps=node.data?.bpSteps?.map((item,index)=>{
      ...
      return (<EBProcessingStepItem key={index} node={node} stepIndex={index}/>)
    });

    return (
      <>
        <Row className="param-panel-row"  gutter={24}>
            <Col className="param-panel-row-label" span={10}>Writeoff ModelID</Col>
        ...
        <Row className="param-panel-row"  gutter={24}>
            <Col className="param-panel-row-label" span={10}>Group ModelID</Col>
            <Col className="param-panel-row-input" span={14}>
                <Input value={node.data.matchGroupModelID} onChange={(e)=>onNodeDataChange({...node.data,matchGroupModelID:e.target.value})}/>
            </Col>
        </Row>
        <Row className="param-panel-row"  gutter={24}>
            <Col className="param-panel-row-label" span={10}>Deal Amount</Col>
            <Col className="param-panel-row-input" span={14}>
        ...
        <Row className="param-panel-row" gutter={24}>
            <Col className="param-panel-row-label" span={10}>Deal Quantity</Col>
            <Col className="param-panel-row-input" span={14}>
