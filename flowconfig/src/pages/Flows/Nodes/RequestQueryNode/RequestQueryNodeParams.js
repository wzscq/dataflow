...
import RequestQueryModelItem from './RequestQueryModelItem';

export default function RequestQueryNodeParams({node}){
    ...
    const models=node.data?.models?.map((item,index)=>{
      ...
      return (<RequestQueryModelItem key={index} node={node} modelIndex={index}/>)
    });
